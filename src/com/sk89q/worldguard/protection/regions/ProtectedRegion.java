...
public abstract class ProtectedRegion implements Comparable<ProtectedRegion> {
     ...
     * List of coowners.
     */
    private DefaultDomain coowners = new DefaultDomain();
     ...
     * @return the coowners
     */
    public DefaultDomain getCoOwners() {
     ...
     * @param owners the owners to setFlag
     */
    public void setCoOwners(DefaultDomain coowners) {
    ...
    public boolean hasMembersOrOwners() {
        return owners.size() > 0 || coowners.size() > 0 || members.size() > 0;
    }
     ...
     */
    public boolean isOwnerOnly(LocalPlayer player) {
    ...
    public boolean isOwner(LocalPlayer player) {
        if (isOwnerOnly(player) || coowners.contains(player)) {
            return true;
        }

        ProtectedRegion curParent = getParent();
        while (curParent != null) {
            if (curParent.getCoOwners().contains(player)) {
                return true;
            }
     ...
     * @return whether an owner
     */
    public boolean isOwnerOnly(String playerName) {
    ...
    public boolean isOwner(String playerName) {
        if (coowners.contains(playerName) || isOwnerOnly(playerName)) {
            return true;
        }

        ProtectedRegion curParent = getParent();
        while (curParent != null) {
            if (curParent.getCoOwners().contains(playerName)) {
                return true;
            }
